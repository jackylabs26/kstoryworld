#!/usr/bin/env bash
# board-routine/kdrama-kickoff.sh
#
# Daily K-Drama content kickoff for KStoryWorld. Invoked by the Paperclip
# routine "K-Drama Daily Content (4-6 KST Random)" with
# `bash _default/board-routine/kdrama-kickoff.sh "$PAPERCLIP_TASK_ID"`.
#
# Routine cron is `0 4 * * *` Asia/Seoul; this script applies a 0..119 minute
# random sleep so the actual fire is uniformly distributed across the
# 04:00..06:00 KST window. Set KDRAMA_KICKOFF_NO_SLEEP=1 to bypass for
# manual dry-runs.
#
# Responsibilities:
#   1. Sleep 0..119 minutes (skippable).
#   2. Pick the day's K-Drama keyword from the rotating pool.
#   3. Call the kdrama-content-generator n8n webhook (or skip in dry-run mode).
#   4. Persist the dry-run sample artifact under
#      n8n-workflows/_dryrun-samples/ for offline inspection.
#
# Exit codes:
#   0  success (network or dry-run)
#   1  webhook call failed
#   2  self-check did not reach 12/12

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=lib.sh
source "$SCRIPT_DIR/lib.sh"

TASK_ID="${1:-${PAPERCLIP_TASK_ID:-manual}}"

if [[ "${KDRAMA_KICKOFF_NO_SLEEP:-0}" != "1" && "${N8N_DRYRUN:-0}" != "1" ]]; then
  SLEEP_MIN=$(( RANDOM % 120 ))
  SLEEP_SEC=$(( SLEEP_MIN * 60 ))
  echo "[kdrama-kickoff] task=$TASK_ID sleeping ${SLEEP_MIN}m to spread 04:00-06:00 KST window"
  sleep "$SLEEP_SEC"
fi

DATE="$(br_today_seoul)"
KEYWORD="$(br_pick_kdrama_keyword)"
SLUG="$(printf '%s' "$KEYWORD" | tr ' ' '-' | tr '[:upper:]' '[:lower:]')"

PAYLOAD=$(cat <<JSON
{
  "task_id": "$TASK_ID",
  "category": "k-drama",
  "keyword": "$KEYWORD",
  "tone": "에디터 큐레이션",
  "language_pair": ["ko", "en"],
  "sections": ["intro","plot_snapshot","cultural_context","why_watch","closing","source"],
  "guards": ["12-check", "no-ai-copy", "spoiler-safety"]
}
JSON
)

echo "[kdrama-kickoff] task=$TASK_ID date=$DATE keyword=$KEYWORD"

RESPONSE_FILE="$(br_dryrun_path "kdrama" "$DATE" "$SLUG")"
mkdir -p "$(dirname "$RESPONSE_FILE")"

if ! RESPONSE=$(n8n_call_kdrama "$PAYLOAD"); then
  echo "[kdrama-kickoff] webhook call failed" >&2
  exit 1
fi

printf '%s\n' "$RESPONSE" > "$RESPONSE_FILE"
echo "[kdrama-kickoff] saved dry-run artifact: $RESPONSE_FILE"

if command -v python3 >/dev/null 2>&1; then
  if ! python3 - "$RESPONSE_FILE" <<'PY'
import json, sys
p = sys.argv[1]
with open(p) as f:
    data = json.load(f)
if isinstance(data, dict) and data.get('dryrun'):
    sys.exit(0)
candidate = data.get('final_output') if isinstance(data, dict) else None
if not isinstance(candidate, dict):
    candidate = data if isinstance(data, dict) else {}
self_check = candidate.get('self_check') if isinstance(candidate, dict) else None
if isinstance(self_check, dict):
    passed = sum(1 for value in self_check.values() if value)
    total = len(self_check)
    if passed == total:
        print(f'[kdrama-kickoff] self-check {passed}/{total} pass')
        sys.exit(0)
    print(f'[kdrama-kickoff] self-check {passed}/{total} FAIL', file=sys.stderr)
    sys.exit(2)
passed = candidate.get('passed') if isinstance(candidate, dict) else None
total = candidate.get('total') if isinstance(candidate, dict) else None
if passed is None or total is None:
    sys.exit(0)
if passed == total:
    print(f'[kdrama-kickoff] self-check {passed}/{total} pass')
    sys.exit(0)
print(f'[kdrama-kickoff] self-check {passed}/{total} FAIL', file=sys.stderr)
sys.exit(2)
PY
  then
    exit 2
  fi
fi

# JAC-1984: optionally open a board-review PR with the dry-run sample.
# Gated by BOARD_AUTO_PR so daily routines don't auto-fire until the board
# opts in. Skips when the response is an N8N_DRYRUN=1 stub (no real draft).
if [[ "${BOARD_AUTO_PR:-0}" == "1" ]]; then
  if command -v python3 >/dev/null 2>&1 && \
     python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if isinstance(d,dict) and d.get("dryrun") else 1)' "$RESPONSE_FILE"; then
    echo "[kdrama-kickoff] dryrun stub — skipping content-pr-adapter"
  else
    echo "[kdrama-kickoff] BOARD_AUTO_PR=1 — handing off to content-pr-adapter"
    "$SCRIPT_DIR/content-pr-adapter.sh" \
      --dryrun-json "$RESPONSE_FILE" \
      --category kdrama \
      --source-issue "${PAPERCLIP_TASK_ID:-JAC-1984}" \
      || echo "[kdrama-kickoff] content-pr-adapter failed (non-fatal)" >&2
  fi
fi
