...
if [[ "${BOARD_AUTO_PR:-0}" == "1" ]]; then
  if command -v python3 >/dev/null 2>&1 && \
    ...
    echo "[kfood-kickoff] dryrun stub — skipping content-pr-adapter"
  else
    echo "[kfood-kickoff] BOARD_AUTO_PR=1 — handing off to content-pr-adapter"
    "$SCRIPT_DIR/content-pr-adapter.sh" \
      --dryrun-json "$RESPONSE_FILE" \
      --category kfood \
      --source-issue "${PAPERCLIP_TASK_ID:-JAC-1984}" \
      || echo "[kfood-kickoff] content-pr-adapter failed (non-fatal)" >&2
  fi
fi
