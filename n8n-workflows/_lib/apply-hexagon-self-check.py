...
def render_self_check_injection(*, category: str, is_anchor: bool, all_text_var: str) -> str:
    ...
    anchor_branch = (
        ...
        if is_anchor
        else f"({all_text_var} || '').includes('/dramas/' + hxId)"
    )
    lines = [
        INJECTION_BEGIN,
