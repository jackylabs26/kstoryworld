...
| `k-food` / `k-pop` / `k-beauty` (sisters), `hexagon_id` set | Body or meta MUST contain the substring `/dramas/<hexagon_id>` at least once. Failure flips `self_check.pass = false` and blocks publish. |
