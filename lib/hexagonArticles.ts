 ...
 *
 * Falls back to whatever non-empty fields are present so partial entries
 * still render something. Self-check #15 is responsible for blocking
 * partial entries upstream.
 */
export function formatImageCredit(image: HexagonImage): string {
  const parts: string[] = [];
...
/** Render the full credit/license footer block for an article. */
export function formatCreditFooter(images: HexagonImage[]): string {
  if (!images || images.length === 0) return '';
  return images
    .map((im, idx) => {
      const role = im.role === 'hero' ? 'Hero' : `Inline ${idx}`;
      return `${role}: ${formatImageCredit(im)}`;
    })
    .join('\n');
}
